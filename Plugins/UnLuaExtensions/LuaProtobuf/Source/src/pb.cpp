...
#define pushinteger(n, u) lpb_pushinteger((L), (n), (u), LS->int64_mode)
    ...
    case PB_Tsint64:
        ...
        case PB_Tint32:
            pushinteger((int32_t)v.u64, 0);
            break;
        case PB_Tuint32:
            pushinteger((uint32_t)v.u64, 1);
            break;
        case PB_Tsint32:
            pushinteger(pb_decode_sint32((uint32_t)v.u64), 0);
            break;
        case PB_Tint64:
            pushinteger((int64_t)v.u64, 0);
            break;
        case PB_Tuint64:
            pushinteger((uint64_t)v.u64, 1);
            break;
        case PB_Tsint64:
            pushinteger(pb_decode_sint64(v.u64), 0);
            break;
        }
        break;
    case PB_Tfloat:
...
static int Lio_read(lua_State *L)
{
    ...
    if (fp != stdin)
        ...
        (void)pb_setmode(pb_fileno(stdin), O_TEXT);
    if (ret != LUA_OK)
    {
        lua_pushnil(L);
        lua_insert(L, -2);
...
static int Lconv_decode_float(lua_State *L)
{
    return lua_pushnumber(L,
                          pb_decode_float((uint32_t)lpb_checkinteger(L, 1))),
           1;
}
...
static int Lpb_loadfile(lua_State *L)
{
    ...
    do
    {
        ...
        if (d == NULL)
            return fclose(fp), luaL_error(L, "out of memory");
...
static int lpb_pushtype(lua_State *L, const pb_Type *t)
{
    ...
    lua_pushstring(L, t->is_map ? "map" : t->is_enum ? "enum"
                                                     : "message");
    return 3;
}
...
static int lpb_pushdeffield(lua_State *L, lpb_State *LS, const pb_Field *f, int is_proto3)
{
    ...
    {
    case PB_Tenum:
        if ((type = f ? f->type : NULL) == NULL)
            return 0;
        if ((f = pb_fname(type, f->default_value)) != NULL)
            ret = LS->enum_as_value ? (lpb_pushinteger(L, f->number, 1, LS->int64_mode), 1) : (lua_pushstring(L, (const char *)f->name), 1);
        else if (is_proto3)
            ret = (f = pb_field(type, 0)) == NULL || LS->enum_as_value ? (lua_pushinteger(L, 0), 1) : (lua_pushstring(L, (const char *)f->name), 1);
        break;
    case PB_Tmessage:
...
static void lpb_setdeffields(lua_State *L, lpb_State *LS, const pb_Type *t, lpb_DefFlags flags)
{
    ...
    {
        int has_field = f->repeated ? (flags & USE_REPEAT) && (t->is_proto3 || LS->decode_default_array) && (lua_newtable(L), 1) : !f->oneof_idx && (f->type_id != PB_Tmessage ? (flags & USE_FIELD) : (flags & USE_MESSAGE) && LS->decode_default_message) && lpb_pushdeffield(L, LS, f, t->is_proto3);
        if (has_field)
...
static void lpbD_rawfield(lpb_Env *e, const pb_Field *f)
{
    ...
    case PB_Tenum:
        ...
        if (ev)
            ...
            lpb_pushinteger(L, (lua_Integer)u64, 1, lpb_lstate(L)->int64_mode);
        if (e->LS->use_dec_hooks)
            lpb_usedechooks(L, e->LS, f->type);
        break;
...
{
    return lpbD_decode(L, lua_isnoneornil(L, 2) ? pb_lslice(NULL, 0) : lpb_checkslice(L, 2), 3);
}
...
void lpb_pushunpackdef(lua_State *L, lpb_State *LS, const pb_Type *t, pb_Field **l, int top)
{
    ...
    if (mode != LPB_COPYDEF && mode != LPB_METADEF)
        return;
