...
namespace UnLua
{
    ...
    template <typename T>
    bool IsType(lua_State *L, int32 Index, TType<T>)
    {
     ...
     */
    template <typename T>
    T Get(lua_State *L, int32 Index, TType<T>);
    ...
    template <typename T>
    TSubclassOf<T> Get(lua_State *L, int32 Index, TType<TSubclassOf<T>>);
    template <typename T>
    TSubclassOf<T> *Get(lua_State *L, int32 Index, TType<TSubclassOf<T> *>);
    template <typename T>
    TSubclassOf<T> &Get(lua_State *L, int32 Index, TType<TSubclassOf<T> &>);
    template <typename T>
    const TSubclassOf<T> &Get(lua_State *L, int32 Index, TType<const TSubclassOf<T> &>);

    template <template <typename, ESPMode> class SmartPtrType, typename T, ESPMode Mode>
    SmartPtrType<T, Mode> Get(lua_State *L, int32 Index, TType<SmartPtrType<T, Mode>>);
    template <template <typename, ESPMode> class SmartPtrType, typename T, ESPMode Mode>
    SmartPtrType<T, Mode> *Get(lua_State *L, int32 Index, TType<SmartPtrType<T, Mode> *>);
    template <template <typename, ESPMode> class SmartPtrType, typename T, ESPMode Mode>
    SmartPtrType<T, Mode> &Get(lua_State *L, int32 Index, TType<SmartPtrType<T, Mode> &>);
    template <template <typename, ESPMode> class SmartPtrType, typename T, ESPMode Mode>
    const SmartPtrType<T, Mode> &Get(lua_State *L, int32 Index, TType<const SmartPtrType<T, Mode> &>);

    template <typename T, typename Allocator>
    TArray<T, Allocator> Get(lua_State *L, int32 Index, TType<TArray<T, Allocator>>);
    template <typename T, typename Allocator>
    TArray<T, Allocator> *Get(lua_State *L, int32 Index, TType<TArray<T, Allocator> *>);
    template <typename T, typename Allocator>
    TArray<T, Allocator> &Get(lua_State *L, int32 Index, TType<TArray<T, Allocator> &>);
    template <typename T, typename Allocator>
    const TArray<T, Allocator> &Get(lua_State *L, int32 Index, TType<const TArray<T, Allocator> &>);

    template <typename T, typename KeyFunc, typename Allocator>
    TSet<T, KeyFunc, Allocator> Get(lua_State *L, int32 Index, TType<TSet<T, KeyFunc, Allocator>>);
    template <typename T, typename KeyFunc, typename Allocator>
    TSet<T, KeyFunc, Allocator> *Get(lua_State *L, int32 Index, TType<TSet<T, KeyFunc, Allocator> *>);
    template <typename T, typename KeyFunc, typename Allocator>
    TSet<T, KeyFunc, Allocator> &Get(lua_State *L, int32 Index, TType<TSet<T, KeyFunc, Allocator> &>);
    template <typename T, typename KeyFunc, typename Allocator>
    const TSet<T, KeyFunc, Allocator> &Get(lua_State *L, int32 Index, TType<const TSet<T, KeyFunc, Allocator> &>);

    template <typename KeyType, typename ValueType, typename Allocator, typename KeyFunc>
    TMap<KeyType, ValueType, Allocator, KeyFunc> Get(lua_State *L, int32 Index, TType<TMap<KeyType, ValueType, Allocator, KeyFunc>>);
    template <typename KeyType, typename ValueType, typename Allocator, typename KeyFunc>
    TMap<KeyType, ValueType, Allocator, KeyFunc> *Get(lua_State *L, int32 Index, TType<TMap<KeyType, ValueType, Allocator, KeyFunc> *>);
    template <typename KeyType, typename ValueType, typename Allocator, typename KeyFunc>
    TMap<KeyType, ValueType, Allocator, KeyFunc> &Get(lua_State *L, int32 Index, TType<TMap<KeyType, ValueType, Allocator, KeyFunc> &>);
    template <typename KeyType, typename ValueType, typename Allocator, typename KeyFunc>
    const TMap<KeyType, ValueType, Allocator, KeyFunc> &Get(lua_State *L, int32 Index, TType<const TMap<KeyType, ValueType, Allocator, KeyFunc> &>);
     ...
     */
    template <bool bCopy>
    FORCEINLINE int32 PushArgs(lua_State *L)
    {
        return 0;
    }
